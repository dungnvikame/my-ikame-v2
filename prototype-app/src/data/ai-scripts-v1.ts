...
/**
 * v1 scripts (S1, S2, S2b deny, S3 manager, S4) + fallback — unchanged behavior from
 * Phase 2/demo-v1, split out verbatim so `ai-scripts.ts` stays under the 200-line budget
 * after Phase 6 added the new module scripts (`ai-scripts-v2.ts`).
 * Deny script (S2b) route list extended with `/community` + `/profile` (Phase 6 req #1)
 * so the permission beat is reachable from the new pages too.
 */
...
const s2bFinanceBudgetDeny: AiScript = {
  ...
  routes: [
    ...
    '/manager/overview', '/manager/team', '/community', '/profile',
  ],
  level: 'A2',
  paragraphs: () => [
...
export const V1_SCRIPTS: AiScript[] = [
